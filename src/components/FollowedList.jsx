...
useEffect(()=>{
  const colRef = collection(db,'users', memberId, 'followed')
 
  // get collection data
 onSnapshot(colRef, (snapshot)=>{
   let followed =[]
   snapshot.docs.forEach((doc)=>{
    followed.push({...doc.data(), id:doc.id})
   })
...
{followed.map(followed=>(
 <li class="flex flex-row" key={followed.id} >
      
 <div 
    onClick={(e)=>{
      e.stopPropagation();
      setMemberId(followed?.id);
 ...
 class="select-none cursor-pointer hover:bg-gray-50 flex flex-1 items-center p-3">
     <div class="flex flex-col w-10 h-10 justify-center items-center mr-4">
       <a href="#" class="block relative">
         <img alt="profile" src={followed?.userImage} class="mx-auto object-cover rounded-full h-9 w-9" />
       </a>
     </div>
     <div class="flex-1 pl-1">
       <div class="text-[13px] dark:text-white text-[#b73838]">{followed?.username}</div>
